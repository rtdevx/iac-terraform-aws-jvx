...
resource "aws_launch_template" "my_launch_template" {
  ...
  vpc_security_group_ids = [ # NOTE: Attach INGRESS SG
    ...
    #aws_security_group.private-ssh.id, # ! SSH SG Removed, implementing SSM. TO BE REMOVED.
    aws_security_group.private-web-8080.id,
    aws_security_group.private-egress.id # NOTE: Attach EGRESS SG

  ]

  # INFO: Attach IAM Instance Profile for SSM
  iam_instance_profile {
    name = aws_iam_instance_profile.ec2_ssm_profile.name
  }

  #key_name  = var.instance_keypair # ! Keypair not required, implementing SSM. TO BE REMOVED.
